from utils.utils import sendF9Marker
import logging
...
class Stimulus:
    ...
    def __init__(self, stimulus, canvas, app, stimulus_id):
        ...
        if self.xType == PIXELS_CALC_METHOD:
            ...
            self.stXStart = self.app.positionDegreesToVSTable[
                                int(self.start_degree)] * self.app.vsWidth / VIRTUAL_SCREEN_LOGICAL_WIDTH
            self.stYStart = int(stimulus["startY"]) * app.vsHeight / VIRTUAL_SCREEN_LOGICAL_HEIGHT
            self.stXEnd = self.app.positionDegreesToVSTable[
                              int(self.end_degree)] * self.app.vsWidth / VIRTUAL_SCREEN_LOGICAL_WIDTH
            self.stYEnd = int(stimulus["endY"]) * app.vsHeight / VIRTUAL_SCREEN_LOGICAL_HEIGHT
        ...
        if self.exitCriteria.lower() == TIME:
            ...
            self.radiusNorm = (self.endShapeRadius - self.startShapeRadius) / (
                        abs(self.stXStart - self.stXEnd) / self.xChange)
    ...
    def init_shape(self, batch_no):
        ...
        if self.xType == PIXELS_CALC_METHOD:
            ...
            self.shapeX = self.app.vsX + self.app.positionDegreesToVSTable[
                int(self.start_degree)] * self.app.vsWidth / 1000
    ...
    def move(self):
        ...
        if self.xType == PIXELS_CALC_METHOD:
            ...
            # transform the location on logic 1000-pixel screen to our real virtual screen size
            self.shapeX = self.app.vsX + self.app.positionDegreesToVSTable[
                int(self.current_degree)] * self.app.vsWidth / 1000
        ...
        adjusted_radius = self.currRadius * \
                          (1 - ((sin(radians(90 - alpha)) * sin(radians(45 - alpha / 2))) / sin(
                              radians(90 + alpha) / 2)))
