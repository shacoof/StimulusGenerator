...
class ClassicCvAbstractTrackingAPI(metaclass=ABCMeta):
    ...
    @abstractmethod  # abstract methods must be overridden
    def _post_process(self, n_frames: int, analysis_data_outputs=None) -> (np.array, bool, np.array):
    ...
    def post_process(self, n_frames: int, analysis_data_outputs=None) -> (np.array, bool, np.array):
        ...
        try:
            outputs = self._post_process(n_frames=n_frames, analysis_data_outputs=analysis_data_outputs)
        except Exception as e:
            print(e)
            traceback.print_tb(e.__traceback__)
        end = time.process_time()
    ...
    def analyse(self, input_frame: np.array, noise_frame: np.array, fps: int, frame_number: int, additional=None) -> (np.array, bool, np.array):
    ...
    # ASK Imri what it's for, if it's generic
    @staticmethod
    def show(n, f):  # show small movie
        if f.shape[0] < 100 or f.shape[1] < 100:
            return cv2.imshow(n, cv2.resize(f, (round(f.shape[0] * 10), round(f.shape[1] * 10))))
        if f.shape[0] < 300 or f.shape[1] < 300:
            return cv2.imshow(n, cv2.resize(f, (round(f.shape[0] * 3), round(f.shape[1] * 3))))
        return cv2.imshow(n, cv2.resize(f, (round(f.shape[0] / 1.5), round(f.shape[1] / 1.5))))
    ...
    def _annotate_frame(self, input_frame: np.array, ok: bool, fps: float, frame_number: int) -> np.array:
        ...
        column = 20
        row = 20
        # in the git it's 0.6 and we heve # between "". ask Imri
        cv2.putText(input_frame, "" + str(int(frame_number)), (column, row), cv2.FONT_HERSHEY_SIMPLEX, 0.4,
                    Colors.GREEN, 2)
        row += 25
