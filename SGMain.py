# USE SGMainApp --> everything was move to there, this is not used
...
def chagneVirtualScreenProperties(direction):
    ...
    if controlMode == constants.LOCATION:
        if direction.lower() == "up":
            ...
            x += 1
            vsX += 1
    elif controlMode == constants.SIZE:
        if direction.lower() == "up":
            vsHeight -= 1
        elif direction.lower() == "down":
            vsHeight += 1
...
def showCrossAndBoundries(event):
    ...
    if xMode:
        ...
        xHorizental = canvas.create_line(vsX, vsY + vsHeight / 2, vsX + vsWidth, vsY + vsHeight / 2, fill=FILL_COLOR,
                                         width=LINE_WIDTH)
        xVertical = canvas.create_line(vsX + vsWidth / 2, vsY, vsX + vsWidth / 2, vsY + vsHeight, fill=FILL_COLOR,
                                       width=LINE_WIDTH)
...
def bind_screen(screen):
    screen.bind('<Up>', processEvent)
    screen.bind('<Down>', processEvent)
    screen.bind('<Left>', processEvent)
    screen.bind('<Right>', processEvent)
    screen.bind(constants.LOCATION, changeControlMode)
    screen.bind(constants.SIZE, changeControlMode)
    screen.bind(constants.UPDATE, updateConfig)
    screen.bind(constants.EXIT, leaveProg)  # todo was e
    screen.bind(constants.RUN, manageStimulus)
    screen.bind(constants.PAUSE, manageStimulus)
    screen.bind(constants.CROSS, showCrossAndBoundries)
